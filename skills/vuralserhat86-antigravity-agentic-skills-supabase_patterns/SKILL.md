...
> **Source:** [Supabase SSR Guide (Next.js)](https://supabase.com/docs/guides/auth/server-side/nextjs) & [Next.js 15 App Router Security](https://nextjs.org/docs/app/building-your-application/data-fetching/server-actions-and-mutations#security)

### Phase 1: Environment & Client Initialization
- [ ] **Client Selection**: Initialize either the Server Client (RSC/Actions) or Browser Client (RCC) with `@supabase/ssr` based on the use case.
- [ ] **Middleware Guard**: In addition to RLS (Row Level Security), implement session checks and session refresh logic in Middleware.
- [ ] **Type Mapping**: Auto-generate `database.types.ts` and inject it into the Supabase instance.

### Phase 2: Data Orchestration
- [ ] **Server-Side Fetching**: Fetch data at the highest level (Page/Layout) in a Server Component, then pass only minimum required data to child components (RCC).
- [ ] **Server Actions**: Define secure actions for form mutations using the `use server` directive and update cache with `revalidatePath`.
- [ ] **RLS Audit**: Verify that `auth.uid()`-based database policies are active for all relevant tables (SELECT/INSERT/UPDATE).

### Phase 3: Security & Real-time Ops
- [ ] **Auth Validation**: Prefer the more secure `getUser()` method instead of `getSession()`.
- [ ] **Real-time Handling**: Set up and clean up (`unmount`) subscriptions via `supabase.channel()` to reflect changes in real time.
- [ ] **Edge Functions**: Use Edge Functions for compute-heavy workflows or third-party API integrations (for example: Stripe/SendGrid).

### Checkpoints
| Phase | Validation |
|-------|------------|
| 1 | Is the Service Role Key ever used in client-side code (RCC)? |
| 2 | Were Optimistic UI updates implemented after form submit? |
| 3 | Are `process.env` values aligned in both local and production (Supabase Dashboard) environments? |
