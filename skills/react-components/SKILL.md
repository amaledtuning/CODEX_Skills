...
## Execution steps
1. **Environment setup**: If `node_modules` is missing, do not run `npm install` automatically. First request explicit user approval for current-task dependency changes, since installation can modify lockfiles and workspace dependency state. Run installation only after approval.
