...
- All active memory writes are rooted under `.codex-memory` unless host/project policy sets `META_MODE_MEMORY_ROOT`.
- Root resolution is portable: default auto-detect + optional `META_MODE_MEMORY_REPO_ROOT` / `META_MODE_MEMORY_ROOT`. Legacy archives under `.agents/memory` are read-only historical inputs unless host/project policy explicitly selects them.
