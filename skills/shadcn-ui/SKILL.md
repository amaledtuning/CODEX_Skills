...
Before running this command, get explicit user confirmation for current-task dependency mutations. It may install or update packages and modify lockfiles/workspace dependency state.
...
Before running `create` or `init`, get explicit user confirmation for current-task dependency/configuration mutations. These commands can change dependency graphs and lockfiles across the workspace.
...
- Do not auto-install dependencies. First request explicit user approval for current-task dependency changes because package installs can modify lockfiles/workspace state.
- After approval, run component installation via CLI to install required dependencies
