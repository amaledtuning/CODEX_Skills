...
- When Browser Use is enabled, runtime-registered, and available for the current session, it MUST be used as the default verification path for local/current-tab targets (`localhost`, `127.0.0.1`, `::1`, `file://`, or the current in-app tab)
- Filesystem presence alone is not enough; runtime registration/current-session availability is required
- If Browser Use fallback is needed, follow `D:\BD\AGENTS.md`; for localhost screenshots use only `D:\BD\tools\capture-localhost-screenshots.ps1` with its narrow approved command shape and state the fallback reason
...
### Step 4.5: Visual Verification (Mandatory When Available)

For local/current-tab targets (`localhost`, `127.0.0.1`, `::1`, `file://`, or the current in-app tab), Browser Use MUST be the default verification path when it is enabled, runtime-registered, and available for the current session.

1. **Check availability**: Confirm Browser Use tools/skill are available
2. **Start dev server**: Use Bash to start a local server (e.g., `npx serve site/public`)
3. **Navigate to page**: Open `http://localhost:3000/{page}.html` in the in-app browser
4. **Capture screenshot**: Capture the rendered page from Browser Use
...
Fallback details are governed by `D:\BD\AGENTS.md`. For localhost/current-tab screenshot fallback, use only `D:\BD\tools\capture-localhost-screenshots.ps1` via the narrow command shape policy and state the reason before proceeding.
