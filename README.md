...
- Cheap read-only/scouting route: `agent_type=default` + `gpt-5.4-mini` + `reasoning_effort=medium`.
- Conditional high-end delegated read-only analysis route: `gpt-5.4` + `reasoning_effort=high` (no writes, no implementation).
